...
async function getMarketData(id: string) {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}/api/markets/${id}`, {
    cache: 'no-store',
  })
...
async function getTicksData(id: string) {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}/api/markets/${id}/ticks`, {
    cache: 'no-store',
  })
  if (!res.ok) return []
