...
impl CLMMInstruction {
    pub fn unpack(input: &[u8]) -> anyhow::Result<CLMMInstruction> {
        Self::try_from_slice(input).map_err(|_| {
            println!("Error: Failed to deserialize instruction data");
            anyhow!("Error: Failed to deserialize instruction data")
        })
    }
}
